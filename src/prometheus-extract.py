...
def search_timeseries(prometheus_connection: PrometheusConnect, query: str, start_time: datetime, end_time: datetime, step: str):
...
def extract_dataset():
    config = Config()
    prometheus_connection = PrometheusConnect(url=config.prometheus_url, disable_ssl=config.prometheus_secure_connection)
    ...
    for column_name, query in queries.items():
        df = search_timeseries(prometheus_connection, query, start_time, end_time, step)
